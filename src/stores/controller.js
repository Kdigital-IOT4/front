  ...
  actions: {
    async controlPositionNext() {
      ...
      if (this.positionNo < this.positionList.length) {
        const value = `$J=G53X${this.positionList[this.positionNo].x}Y${
          this.positionList[this.positionNo].y
        }Z${this.positionList[this.positionNo].z}F${
          this.positionList[this.positionNo].feedrate
        }`;
        this.sendCommand(value);
        if (this.positionList[this.positionNo].pauseTime > 0) {
          const value = `$J=G53X${this.positionList[this.positionNo].x}Y${
            this.positionList[this.positionNo].y
          }Z${this.positionList[this.positionNo].z - 1}F${100}`;
          this.sendCommand(value);
        }
    ...
    async chainingChangePosition(item) {
      ...
      item.cocktailDetailList.forEach(async (item) => {
        var result = this.documents.filter(
          (doc) => doc.bottle?.code == item.code
        )[0];
        if (result) {
          this.positionList.push({
            x: result.x * 800,
